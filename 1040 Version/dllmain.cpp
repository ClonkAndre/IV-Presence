// IV-Presence (1040 compatible) by ItsClonkAndre
// Version 1.0
...
void scriptTick() {
    ...
    STORE_WANTED_LEVEL(player, &wantedLevel);
    GET_CHAR_COORDINATES(playerPed, &pX, &pY, &pZ);
    GET_CHAR_HEALTH(playerPed, &health);
    GET_CHAR_ARMOUR(playerPed, &armor);
    GET_KEY_FOR_CHAR_IN_ROOM(playerPed, &currentRoomKey);

    currentEpisode = GET_CURRENT_EPISODE();
    isDead = IS_CHAR_DEAD(playerPed);
    isDoingAMission = GET_MISSION_FLAG();
...
#pragma region Image Stuff
        switch (currentRoomKey) {
            case -933277074: // Strip Club Bohan
            case -2096494209:
            case 889160557:
                ...
                dPresence.largeImageText = currentLocationBuffer;
                break;
            case -1333686044: // Strip Club Alderney
            case -534648211:
            case -1634702078:
            case 713594162:
...
#pragma region Details
        ...
        if (!isDead) {
            if (IS_CHAR_IN_ANY_CAR(playerPed)) {
                ...
                GET_CAR_CHAR_IS_USING(playerPed, &currentVehicle);

                unsigned int vehicleModel;
                GET_CAR_MODEL(currentVehicle, &vehicleModel);
                ...
                else if (IS_THIS_MODEL_A_HELI(vehicleModel)) {
                    char locationBuffer[256];
            ...
            // State
            if (wantedLevel != 0) {
                if (PLAYER_HAS_FLASHING_STARS_ABOUT_TO_DROP(player)) {
                    if (wantedLevel > 1) {
                    ...
                    if (PLAYER_HAS_GREYED_OUT_STARS(player)) {
                        if (wantedLevel > 1) {
                            char wantedLevelBuffer[256];
                    ...
                    if (currentRadioStation == "None") {
                        if (IS_CHAR_IN_ANY_CAR(playerPed)) {
                            ...
                            if (!Scripting::IS_THIS_MODEL_A_TRAIN(vehicleModel)) {
                                const char* carName = GET_DISPLAY_NAME_FROM_VEHICLE_MODEL(vehicleModel);
